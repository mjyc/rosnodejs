...
var ServiceServer = function (_EventEmitter) {
  ...
  _createClass(ServiceServer, [{
    ...
    value: function _handleMessage(client, data) {
      ...
      var result = this._requestCallback(req, resp);
      Promise.resolve(result).then(function (success) {
        ...
        var serializeResponse = TcprosUtils.serializeServiceResponse(_this4._messageHandler.Response, resp, success);

        // send service response
        client.write(serializeResponse);

        if (!client.$persist) {
          _this4._log.debug('Closing non-persistent client');
          client.end();
          delete _this4._clients[client.name];
        }
      });
    }
  }, {
